      ...
      * {{ box-sizing: border-box; margin: 0; padding: 0; }}
        body {{ background: #0d1117; color: #c9d1d9; font-family: 'Segoe UI', sans-serif; min-height: 100vh; display: flex; flex-direction: column; align-items: center; justify-content: center; }}
          #login-screen {{ background: #161b22; border: 1px solid #30363d; border-radius: 12px; padding: 40px; width: 320px; text-align: center; }}
            #login-screen h2 {{ color: #58a6ff; margin-bottom: 20px; }}
              #pwd-input {{ width: 100%; padding: 10px; background: #0d1117; border: 1px solid #30363d; border-radius: 6px; color: #c9d1d9; font-size: 16px; margin-bottom: 12px; }}
                #login-btn {{ width: 100%; padding: 10px; background: #238636; border: none; border-radius: 6px; color: white; font-size: 16px; cursor: pointer; }}
                      ...
                      #report {{ display: none; width: 100%; max-width: 1000px; padding: 20px; }}
                        .header {{ text-align: center; margin-bottom: 20px; }}
                          .header h1 {{ color: #58a6ff; font-size: 24px; }}
                            .time {{ color: #8b949e; font-size: 13px; margin-top: 4px; }}
                              .gold-bar {{ display: flex; gap: 12px; margin-bottom: 20px; flex-wrap: wrap; }}
                                .gold-item {{ flex: 1; background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 12px; min-width: 140px; }}
                                  .gold-label {{ color: #8b949e; font-size: 12px; }}
                                    .gold-value {{ color: #e3b341; font-size: 18px; font-weight: bold; margin-top: 4px; }}
                                      .summary {{ display: flex; gap: 12px; margin-bottom: 20px; flex-wrap: wrap; }}
                                        .summary-card {{ flex: 1; background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 16px; min-width: 140px; }}
                                                                    ...
                                                                    .mono {{ font-family: monospace; }}
                                                                      .stock-name {{ font-weight: 600; color: #c9d1d9; }}
                                                                        .stock-ticker {{ font-size: 11px; color: #8b949e; margin-top: 2px; }}
                                                                          .market-tag {{ padding: 2px 6px; border-radius: 4px; font-size: 11px; font-weight: bold; }}
                                                                            .tag-tw {{ background: #1f4e3d; color: #3fb950; }}
                                                                              .tag-us {{ background: #1a3a5c; color: #58a6ff; }}
                                                                                .tag-hk {{ background: #4a1e1e; color: #f85149; }}
                                                                                  .tag-jp {{ background: #3a2e1a; color: #e3b341; }}
                                                                                    .badge {{ padding: 3px 8px; border-radius: 10px; font-size: 12px; font-weight: bold; }}
                                                                                      .badge-up {{ background: #1f4e3d; color: #3fb950; }}
                                                                                        .badge-down {{ background: #4a1e1e; color: #f85149; }}
                                                                                        </style>
                                                                                        </head>
                                                                                        <body>
                                                                                        <div id="login-screen">
